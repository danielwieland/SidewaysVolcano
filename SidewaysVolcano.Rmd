---
title: "Sideways Volcano Lung Squamous Seq"
...
df.input = read.table("LUSC_survival.txt", header = TRUE,sep="\t",stringsAsFactors = FALSE) #Phenotype data like survival years etc
df.map = read.table("LungSquamous_Seq", header = FALSE,sep="\t",stringsAsFactors = FALSE,na.strings=c("", "NA")) #Matrix of genes vs patient id
...
```{r gene_search, include=TRUE, error=FALSE}
...
write.table(sbs, file = "LUSC_LMNB1_strong_scaling", append = FALSE, quote = TRUE, sep = "\t",
            eol = "\n", dec = ".", row.names = TRUE,
...
write.table(kmsig2, file = "LUSC_Survival", append = FALSE, quote = TRUE, sep = " ",
...
## Gene search for MATLAB robust regression genes

```{r MATLAB_gene_search, include=FALSE, error=FALSE}
#if you want to colour points on your clipboard onto the figure
...
#how man y genes hat scale strongly predict significant survival
scaling_survival1=subset(LMNB1_km, LMNB1_km[,1]<0.05,LMNB1_km[,5]<1)
...
## Survival Volcano for Robust regression genes

```{r robust_regression_survival_volcano, fig.width=3}
...
# Survival for patient matched tumor/normal ratios
$$
Patient \: matched \; \frac {Tumor} {Adjacent \; uninvolved}
$$
## Matching tumor and adjacent involved tissue samples to the same patient ids 

```{r tumor_normal, include=FALSE, error=FALSE}
# If you want to match patients with their primary tumor and normal tissue respectively
Trk1 <- data.frame(matrix(ncol = 3, nrow = 2))
name=df.input[1] #patient ids from the survival file
namepro=df.map[1,2:dim(df.map)[2]] #patient ids from mrna seq file
patientid=substr(namepro[1],1,12) #pure patient id
y=1
for(i in 1:(dim(df.map)[2]-2))
{
  for(j in (i+1):dim(df.map)[2]-1)
  {
    ...
    {
      if(substr(namepro[1,i],14,15)=='01' && substr(namepro[1,j],14,15)=='11')
      {
        ...
        y=y+1
      }else if(substr(namepro[1,j],14,15)=='01' && substr(namepro[1,i],14,15)=='11')
      {
        patientid[y]=substr(namepro[1,i],1,12)
...
## Matching ratios to survival data

```{r matching_ratios, include=FALSE, error=FALSE}
##matching ratio patient ids to survival data
Trk <- data.frame(matrix(ncol = 3, nrow = 2))
for(i in 1:length(patientid))
{
  for(j in 1:dim(df.input)[1])
...
```{r ratio_gene_search, include=TRUE, error=FALSE}
ratio_gene="COL1A1"
iden=which(df.map$V1==ratio_gene)-1
...
```{r ratio_kaplan_meier}
Trk[,1]=as.double(t(Trk1[iden,]))
Trk[,4]=NULL
Trk[,4] <- Trk[,1]> median(Trk[,1])
fit <- survfit(Surv(Trk[,2],Trk[,3]) ~ Trk[,4],
               data = Trk)
ggsurvplot(fit, data = Trk, risk.table = FALSE,pval = TRUE,xlab = "Survival time in years",font.x=15,font.y=15,font.tickslab=15)
```


## Evaluating ratio patient survival 

Patients are divided into two cohorts (high expressers and low expressers) based on median expression on a particular gene. The median survival of both the cohorts is evaluated. This process is repeated for all genes.

```{r ratio_survival, include=FALSE, error=FALSE}
chk=0.5*dim(Trk1)[2]
kmsig <- data.frame(matrix(ncol = 3, nrow = 2))
pro_name=df.map[1,1]
k=1
...
write.table(kmsig2, file = "TumorName_Ratio_Survival", append = FALSE, quote = TRUE, sep = " ",
            eol = "\n", na = "NA", dec = ".", row.names = TRUE,
            col.names = TRUE, qmethod = c("escape", "double"),
            fileEncoding = "") #Change Tumor Name here
```
...
```{r ratio_MATLAB_gene_search, include=FALSE, error=FALSE}
#if you want to colour points on your clipboard onto the figure
CNN1 <- read.table(pipe("pbpaste"), sep="\t", header = FALSE)
LMNB1_km=kmsig2[1,]
for (i in 1:dim(CNN1)[1])
...
```
Percentage of genes scaling strongly with `r ratio_gene` that predict significantly poor survival=`r per*100`

## Survival Volcano for Robust regression genes

```{r ratio_robust_regression_survival_volcano, fig.width=10, fig.height=7}
plot(-log2(as.double(kmsig2[,1])),log2(as.double(kmsig2[,5])),pch = 19,cex=0.3,col="grey",xlab='-log2(p-value)',ylab='log2(median survival factor change)')
points(-log2(LMNB1_km[,1]),log2(LMNB1_km[,5]),pch = 19,cex=0.5,col="red")
points(c(-log2(0.05),-log2(0.05),-log2(0.05)),-1:1,pch = 3,cex=1)
```